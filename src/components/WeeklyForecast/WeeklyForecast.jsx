...
export const WeeklyForecast = ({ daily, timezone }) => {
  ...
  const getDayInfo = (day, index) => {
    ...
    return { formattedDate, dayName, isToday, background, isDay };
  };

  return (
    <section className={styles.weekly}>
      <div className={styles.weekly__wrapper}>
        <h2 className={styles.weekly__title}>Weekly Forecast</h2>
        <div className={styles.weekly__list}>
          {daily.map((day, index) => {
            const { formattedDate, dayName, isToday, background, isDay } = getDayInfo(day, index);
